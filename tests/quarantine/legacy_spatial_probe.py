#!/usr/bin/env python
"""
Legacy spatial data probe.

Quarantined from automated discovery because it expects preloaded spatial data
in a non-isolated database and is better treated as an ad hoc verification
script than a repeatable unit/integration test.
"""
...
def test_top_rivers():
    ...
    rivers_by_size = sorted(
        rivers,
        key=lambda r: abs(r.geom.extent[2] - r.geom.extent[0])
        + abs(r.geom.extent[3] - r.geom.extent[1]),
        reverse=True,
    )
    ...
    for boundary in rivers_by_size[:3]:
        ...
        assert boundary.geom.extent is not None, (
            f"River {boundary.name} geometry has no extent"
        )
